...
class Auth:
    ...
    def create_access_token(self, data: dict[str, Any], expires_delta: Optional[float] = None):
        to_encode = data.copy()
        if expires_delta:
    ...
    def create_refresh_token(self, data: dict[str, Any], expires_delta: Optional[float] = None):
        to_encode = data.copy()
        if expires_delta:
            expire = datetime.now(timezone.utc) + timedelta(seconds=expires_delta)
        else:
    ...
    def get_current_user(self, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db_session)) -> repository_users.User:
